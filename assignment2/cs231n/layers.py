...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    - bn_param: Dictionary w    data at test-time.
ith the following keys:
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad = conv_param['pad']
    pad_x = np.pad(x, pad_width=((0, 0), (0, 0), (pad, pad), (pad, pad)), mode='constant')
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    H_out = 1 + (H + 2 * pad - HH) // stride
    W_out = 1 + (W + 2 * pad - WW) // stride
    fileters_out = np.zeros((F, H_out, W_out))
    single_out = np.zeros((H_out, W_out))
        ...
        for j in range(F):
            ...
            for start_h in range(0, H, stride):
                for start_w in range(0, W, stride):
                    if ((start_h + HH <= H + (2 * pad)) and (start_w + WW <= W + (2 * pad))):
                        h_dot = start_h // stride
                        w_dot = start_w // stride
                        single_compute = np.sum(pad_x[i, :, start_h:start_h + HH, start_w:start_w + WW] * w[j])
                        single_out[h_dot, w_dot] = single_compute

            fileters_out[j, :, :] = single_out + b[j]
        out[i, :, :, :] = fileters_out
