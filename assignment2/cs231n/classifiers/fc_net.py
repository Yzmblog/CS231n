...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        dim_before = input_dim
        for i, hd in enumerate(hidden_dims):
            self.params['W%d' % (i + 1)] = weight_scale * np.random.randn(dim_before, hd)
            self.params['b%d' % (i + 1)] = np.zeros(hd)
            if normalization :
                self.params['gama%d' % (i + 1)] = np.ones(hd)
                self.params['beta%d' % (i + 1)] = np.zeros(hd)
            dim_before = hd
        self.params['W%d' % self.num_layers] = weight_scale * np.random.randn(dim_before, num_classes)
